...
// 파일명을 위한 상세 타임스탬프 (YYYY-MM-DD(요일)_HH시MM분)
const nowTimestampString = () => {
    ...
    return `${year}-${month}-${date}(${day})_${hours}시${minutes}분`;
};
...
const App: React.FC = () => {
  ...
  const [loadedFileName, setLoadedFileName] = usePersistentState<string>('loaded_file_name_v1', '');
  ...
  const handleSaveData = () => {
    const performSave = () => {
      try {
        ...
        // 요청대로 파일명 형식을 [교회이름 헌금관리]_날짜(요일)_시간 으로 변경
  ...
  const handleLoadMembersOnly = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const performLoad = () => {
        ...
        reader.onload = (e) => {
            try {
                ...
                setLoadedFileName(`(성도 전용) ${fileName}`);
    ...
    const handleLoadSnapshot = (snapshotData: DataSnapshot['data'], timestamp: string) => {
      if (window.confirm('저장된 데이터를 불러오면 현재 작업 내용이 모두 덮어쓰여집니다. 계속하시겠습니까?')) {
          ...
          setLoadedFileName(`내부 저장 목록 (${new Date(timestamp).toLocaleString('ko-KR')})`);
          setView('main');
          alert('데이터를 성공적으로 불러왔습니다.');
      }
    };
...
const ExcelExportModal: React.FC<{
    ...
    const handleExport = () => {
        ...
        // 엑셀 파일명 형식도 [교회이름 헌금관리]_구분_날짜(요일)_시간.xlsx 로 변경
...
const SnapshotsModal: React.FC<{
    ...
    onClose: () => void;
    onLoad: (snapshotData: DataSnapshot['data'], timestamp: string) => void;
                    ...
                    {snapshots.map(snapshot => (
                        <li key={snapshot.timestamp} className="snapshot-item">
                            ...
                            <div className="snapshot-actions">
                                <button onClick={() => onLoad(snapshot.data, snapshot.timestamp)} className="load-btn">불러오기</button>
